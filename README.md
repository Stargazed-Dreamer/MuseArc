...
MuseArc 是面向超大、混乱歌曲库的清洗与统一管理桌面工具。可与 MusePlayer 协同工作。
...
`tools/chromaprint/bin/` 下的 Windows DLL 来自 Chromaprint (LGPL-2.1)、FFTW (GPL-2.0)、GCC runtime (GPL-3.0 + GCC Exception)、winpthreads (MIT)。各组件许可证全文见 [THIRD_PARTY_LICENSES.md](tools/chromaprint/bin/THIRD_PARTY_LICENSES.md)。Linux/macOS 依赖系统库,不分发这些 DLL。
